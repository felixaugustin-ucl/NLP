...
flags.DEFINE_string('embedding_path', '', 'path to node embeddings parquet/csv file')
flags.DEFINE_string('edge_path', '', 'path to edge parquet/csv file')
...
def _dataset_name():
    if FLAGS.train_prefix:
        parts = FLAGS.train_prefix.rstrip("/").split("/")
        return parts[-2] if len(parts) > 1 else parts[-1]
    if FLAGS.embedding_path:
        return os.path.splitext(os.path.basename(FLAGS.embedding_path))[0]
    return "data"

def log_dir():
    log_dir = FLAGS.base_log_dir + "/unsup-" + _dataset_name()
...
def _write_embedding_artifacts(out_dir, stem, mod, embeddings, nodes):
    np.save(os.path.join(out_dir, stem + mod + ".npy"), embeddings)
    with open(os.path.join(out_dir, stem + mod + ".txt"), "w") as fp:
        fp.write("\n".join(map(str, nodes)))
    ...
    while not finished:
        ...
        for i, edge in enumerate(edges):
            if not edge[0] in seen:
                embeddings.append(outs_val[-1][i,:])
                nodes.append(edge[0])
                seen.add(edge[0])
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    embeddings = np.vstack(embeddings)
    _write_embedding_artifacts(out_dir, "train", mod, embeddings, nodes)
    # Keep the legacy val.* names so older notebooks still load existing runs.
    _write_embedding_artifacts(out_dir, "val", mod, embeddings, nodes)
...
def load_embedding_edge_data(embedding_path, edge_path):
    embeddings = _read_table(embedding_path)[["node_id", "embedding"]].drop_duplicates("node_id").reset_index(drop=True)
    edges = _read_table(edge_path)[["src", "dst"]]

    embeddings["embedding"] = embeddings["embedding"].map(lambda value: np.asarray(value, dtype=np.float32))
    features = np.vstack(embeddings["embedding"].values)
    id_map = {node_id: idx for idx, node_id in enumerate(embeddings["node_id"].tolist())}

    G = nx.Graph()
    for node_id in id_map:
        G.add_node(node_id)
        G.node[node_id]["val"] = False
        G.node[node_id]["test"] = False

    for row in edges.itertuples(index=False):
        if row.src not in id_map or row.dst not in id_map:
            continue
        G.add_edge(row.src, row.dst)
        G[row.src][row.dst]["train_removed"] = False

    return G, features, id_map, list(G.edges()), {}
...
def main(argv=None):
    ...
    if FLAGS.embedding_path and FLAGS.edge_path:
        train_data = load_embedding_edge_data(FLAGS.embedding_path, FLAGS.edge_path)
    else:
